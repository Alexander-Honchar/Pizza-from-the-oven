...
const arrayOrderDetails = [];
const orderStatusAccepted = "Принят";
const orderStatusInWork = "В работе";
const orderStatusReady = "Готов";
const orderStatusASent = "Отправлен";
...
function checkAllStatus(arrayAllStatus) {
...
function requestChangeStatus(id, status) {
...
function changeArrayOrders(newArray) {
...
function plusNewOrder(response) {
    for (var i = 0; i < response.length; i++) {
        let tablestring = [];
        tablestring[0] = response[i].street == null ? '' : " " + String(response[i].street);
        tablestring[1] = response[i].house == null ? '' : " дом." + String(response[i].house);
